...
import { sendCreditAnalysisStatusEmail } from "@/lib/email";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existingRows = await db
      ...
      .where(eq(creditAnalysisRequests.id, input.id))
      .limit(1);
    ...
    const [updated] = await db
      ...
      .where(eq(creditAnalysisRequests.id, input.id))
      .returning({ id: creditAnalysisRequests.id, status: creditAnalysisRequests.status });

    if (existing.status !== input.status && existing.requestedByEmail) {
      const emailResult = await sendCreditAnalysisStatusEmail({
    ...
    console.error("Failed to update credit analysis status:", error);
    const message =
